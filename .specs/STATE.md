...
**Where**: BUILD fix round 3 closed the full and AppImage-only missing/invalid version boundary proof gap at `92ec965`; all four new cases passed, the Coverage join maps all four Landing doors with door 4 covered by C8/C9/C19, and the six-file packaging suite passed (88 passed, 4 skipped). C5, C6, C22 pending external actions.
**In progress**: none
**Next step**: Independent VERIFY over `ee07590..HEAD` with every check; re-evaluate C2/C3/C8/C9/C14/C19 and the supplemental wildcard/error-path/version-boundary proofs, and retain the external C5/C6/C22 status. Release role: push `fix/linux-app` and dispatch `release-linux.yml` dry-run (C5). User: Arch/Mesa 26 host smoke (C6). After first release: C22 download-URL proof.
