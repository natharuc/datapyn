...
| Successful reruns with known unversioned aliases already in their output directory (added during BUILD fix round 2) | remove the five Linux aliases and `DataPyn-macos-arm64.dmg` before producing the versioned outputs | Maestro decision: criteria 7, 8, and 18 require unconditional alias absence after a successful run, so those checked outcomes override the earlier unconfirmed stale-alias assumption; only the known filenames are removed, while unrelated output files remain untouched | y |
...
| 4. Successful packaging reruns clear known local aliases (added during BUILD fix round 2) | `package.sh` removes `datapyn_amd64.deb`, `datapyn-x86_64.rpm`, `datapyn-x86_64.pkg.tar.zst`, `DataPyn-x86_64.AppImage`, and `DataPyn-linux-x86_64.tar.gz` from its output directory; `package_dmg.sh` removes `DataPyn-macos-arm64.dmg` from the repository root before writing the versioned DMG | leaving aliases because workflow uploads use the printed list - rejected by the Maestro: the approved checks require those aliases to be absent after every successful rerun |
