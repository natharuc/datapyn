...
def test_settings_dialog_shows_all_shortcuts(qapp):
    ...
    expected_shortcuts = [
        'execute_sql', 'execute_all', 'clear_results',
        'new_file', 'open_file', 'save_file', 'save_as',
        'new_tab', 'close_tab',
        'find', 'replace',
        'manage_connections', 'new_connection',
        'settings'
    ]
    
    assert row_count >= len(expected_shortcuts), f"Esperado pelo menos {len(expected_shortcuts)} atalhos, encontrado {row_count}"
