...
class TestAutoUpdateService:
    ...
    def test_has_pending_update_validates_file(self, auto_update_service, tmp_path):

        zip_path = tmp_path / "DataPyn-1.1.0-windows.zip"

        zip_path.write_bytes(b"PK")

        auto_update_service.save_pending_update("1.1.0", str(zip_path))

        assert auto_update_service.has_pending_update() is True
    ...
    @patch("src.services.auto_update_service.launch_setup_update", return_value=(True, ""))

    def test_apply_pending_update_launches_setup(self, mock_launch, auto_update_service, tmp_path):
        ...
        ok, err = auto_update_service.apply_pending_update()
    ...
    def test_apply_pending_update_fails_without_package(self, auto_update_service):

        with patch.object(auto_update_service, "has_pending_update", return_value=False):
